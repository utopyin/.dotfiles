...
export class McpServerManager {
  ...
  private async createConnection(
    ...
    try {
      if (definition.command) {
        ...
        transport = await McpServerManager.createHttpTransport(
          definition,
          name
        );
      } else {
  ...
  private static async createHttpTransport(
    ...
    if (definition.auth === "bearer") {
      ...
      if (!token) {
        ...
        throw new Error(
          `Missing bearer token for MCP server "${serverName ?? urlText}" (${missingSource})`
        );
      }
      ...
      authProvider = new McpOAuthProvider(
        ...
        },
        {
          onRedirect: async (_url) => {
            ...
            logger.debug(
              `${serverName}: oauth redirect requested: ${_url.toString()}`
            );
          },
        }
      );
    ...
    const transports: {
      ...
      {
        factory: () =>
          new SSEClientTransport(url, hasOpts ? transportOpts : undefined),
        name: "SSE",
      },
    ];
    ...
    for (const { name, factory } of transports) {
      ...
      try {
        ...
        logger.debug(
          `${serverName ?? "?"}: ${name} transport failed: ${lastError.message}`
        );
    ...
    if (transportErrors.length > 0) {
      throw new Error(
        `All transports failed for "${serverName ?? urlText}": ${transportErrors.join(" | ")}`
      );
    }
  ...
  private static async fetchAllResources(
    client: Client
  ): Promise<McpResource[]> {
    try {
  ...
  private attachAdapterNotificationHandlers(
    ...
    client.setNotificationHandler(
      ...
      (notification) => {
        const listener = this.uiStreamListeners.get(
          notification.params.streamToken
        );
        if (!listener) {
          return;
        }
