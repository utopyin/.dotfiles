...
export const renderDirectToolCall =
  (spec: DirectToolSpec): NonNullable<ToolDefinition["renderCall"]> =>
  (args, theme) => {
    const recordArgs =
...
export const resolveDirectTools = (
  config: McpConfig,
...
export const createDirectToolExecutor = (
  ...
  async function execute(_toolCallId, params) {
    ...
    if (!connected) {
      ...
      const failureSuffix =
        failedAgo === null ? "" : ` (failed ${failedAgo}s ago)`;
      return {
        content: [
          {
            text: `MCP server "${spec.serverName}" not available${failureSuffix}`,
            type: "text" as const,
          },
        ],
    ...
    try {
      ...
      if (
        isAuthFailure &&
        state.config.mcpServers[spec.serverName]?.auth === "oauth"
      ) {
        ...
        return {
          content: [
            {
              text: `Authentication expired for "${spec.serverName}". Run /mcp-auth ${spec.serverName} to re-authenticate.`,
              type: "text" as const,
            },
          ],
