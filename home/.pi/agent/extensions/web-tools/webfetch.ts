...
export const createWebFetchTool = () => ({
	...
	renderCall(args: { format?: WebFetchFormat; url: string }, theme: RenderTheme) {
	...
	renderResult(result: WebFetchResult, options: ToolRenderOptions, theme: RenderTheme) {
...
const createImageFetchResult = (input: {
	...
	return {
		content: [
			...
			imageContent(input.buffer.toString("base64"), input.parsedContentType.mime),
		],
		details,
	};
};
...
const createTextFetchResult = async (input: {
	...
	finalUrl: URL;
	format: WebFetchFormat;
	parsedContentType: { charset?: string; contentType: string; kind: string; mime: string };
	requestedUrl: URL;
	status: number;
}) => {
...
const convertHtmlToMarkdown = (decodedText: string, finalUrl: string): string => {
