...
export const executeUiMessages = (
  ...
  if (sessions.length === 0) {
    return {
      content: [
        { text: "No UI session messages available.", type: "text" as const },
      ],
      details: { sessions: 0 },
    };
  }
  ...
  try {
    if (regex) {
      ...
      if (terms.length === 0) {
        return {
          content: [
            { text: "Search query cannot be empty", type: "text" as const },
          ],
...
export const executeList = (
  ...
  if (toolNames.length === 0) {
    if (connection?.status === "connected") {
      return {
        content: [
          { text: `Server "${server}" has no tools.`, type: "text" as const },
        ],
...
export const executeConnect = async (
  state: McpExtensionState,
  serverName: string
  ...
  try {
    ...
    if (error instanceof NeedsAuthError) {
      ...
      return {
        content: [
          {
            text: `Server "${serverName}" requires OAuth authentication. Run /mcp-auth ${serverName} to authenticate.`,
            type: "text" as const,
          },
        ],
...
export const executeCall = async (
  ...
  if (!serverName && !toolMeta && prefixMode !== "none") {
    ...
    for (const { name: configuredServer } of candidates) {
      ...
      toolMeta = findToolByName(
        state.toolMetadata.get(configuredServer),
        toolName
      );
      if (toolMeta) {
  ...
  try {
    ...
    if (toolMeta.resourceUri) {
      ...
      return {
        content:
          content.length > 0
            ? content
            : [{ text: "(empty resource)", type: "text" as const }],
        details: {
          mode: "call",
    ...
    return {
      content:
        content.length > 0
          ? content
          : [{ text: "(empty result)", type: "text" as const }],
      details: {
        mcpResult: result,
        mode: "call",
        server: serverName,
    ...
    if (
      isAuthFailure &&
      serverName &&
      state.config.mcpServers[serverName]?.auth === "oauth"
    ) {
      state.needsAuth.add(serverName);
      ...
      return {
        content: [
          {
            text: `Authentication expired for "${serverName}". Run /mcp-auth ${serverName} to re-authenticate.`,
            type: "text" as const,
          },
        ],
