...
const disposeMcpState = async (current: McpExtensionState | null, reason: string): Promise<void> => {
    if (!current) {
        return;
    }
    if (current.uiServer) {
...
const mcpAdapter = (pi: ExtensionAPI) => {
    ...
    for (const spec of directSpecs) {
        pi.registerTool({
            ...
            parameters: Type.Unsafe<Record<string, unknown>>(spec.inputSchema || { properties: {}, type: "object" }),
    ...
    pi.registerCommand("mcp", {
        ...
        getArgumentCompletions: (inputPrefix: string) => {
            ...
            if (parts.length <= 1) {
                ...
                const filtered = inputPrefix ? subcommands.filter(i => i.value.startsWith(inputPrefix)) : subcommands;
            ...
            if (parts[0] === "reconnect" && parts.length <= 2) {
                ...
                const items = servers.map(name => ({ label: name, value: `reconnect ${name}` }));
        ...
        handler: async (args, ctx) => {
            ...
            await (ctx.hasUI ? openMcpPanel(state, pi, ctx, earlyConfigPath) : showStatus(state, ctx));
        },
    });
    pi.registerCommand("mcp-auth", {
        ...
        getArgumentCompletions: (inputPrefix: string) => {
            ...
            if (parts.length <= 1) {
                const items = [
                    ...servers.map(name => ({ label: `${name}  (authenticate)`, value: name })),
                    { label: "remove  (clear credentials)", value: "remove" },
                ];
                const filtered = inputPrefix ? items.filter(i => i.value.startsWith(inputPrefix)) : items;
                return filtered.length > 0 ? filtered : null;
            }
            ...
            if (parts[0] === "remove" && parts.length <= 2) {
                ...
                const items = servers.map(name => ({ label: name, value: `remove ${name}` }));
                const filtered = partial ? items.filter(i => i.label.startsWith(partial)) : items;
                return filtered.length > 0 ? filtered : null;
            }
            return null;
        ...
        handler: async (args, ctx) => {
            ...
            if (!subcommand) {
                if (ctx.hasUI) {
                    ctx.ui.notify("Usage: /mcp-auth <server-name>\n       /mcp-auth remove <server-name>", "error");
                }
                return;
            }
            if (!state && initPromise) {
            ...
            if (params.args) {
                try {
                    ...
                    if (typeof parsedArgs !== "object" || parsedArgs === null || Array.isArray(parsedArgs)) {
                        ...
                        return {
                            content: [{ text: `Invalid args: expected a JSON object, got ${gotType}`, type: "text" as const }],
                    ...
                    return {
                        content: [{ text: `Invalid args JSON: ${error instanceof Error ? error.message : error}`, type: "text" as const }],
