...
import type {
  UiDisplayMode,
  UiDisplayModeRequest,
  UiDisplayModeResult,
  UiHostContext,
  UiMessageParams,
  UiModelContextParams,
  UiOpenLinkResult,
  UiProxyRequestBody,
  UiProxyResult,
  UiResourceContent,
...
export const startUiServer = async (
  ...
  let currentDisplayMode: UiDisplayMode =
    options.hostContext?.displayMode ?? "inline";
  let nextEventId = 1;
  const eventLog: {
    id: number;
    name: string;
  ...
  const getLatestCheckpointIndex = () => {
    for (let index = eventLog.length - 1; index >= 0; index -= 1) {
      ...
      if (
        envelope?.frameType === "checkpoint" ||
        envelope?.frameType === "final"
      ) {
        return index;
      }
    }
  ...
  const replayEvents = (
    ...
    const parsedLastId = lastEventIdHeader
    ...
    const eventsToReplay = Number.isFinite(parsedLastId)
      ...
      : (() => {
          ...
          return latestCheckpointIndex >= 0
            ? eventLog.slice(latestCheckpointIndex)
            : eventLog;
        })();
    ...
    try {
      ...
      if (method === "GET" && url.pathname === "/") {
        ...
        const html = buildHostHtmlTemplate({
          allowAttribute: buildAllowAttribute(
            options.resource.meta.permissions
          ),
          cacheToolConsent: options.consentManager.shouldCacheConsent(),
          hostContext,
          requireToolConsent: options.consentManager.requiresPrompt(
            options.serverName
          ),
      ...
      if (url.pathname === "/proxy/ui/open-link") {
        ...
        const result: UiOpenLinkResult = URL.canParse(openParams.url)
          ? {}
          : { isError: true };
        sendJson(res, 200, { ok: true, result });
        return;
      ...
      if (url.pathname === "/proxy/ui/complete") {
        ...
        const reason =
          typeof completeParams.reason === "string"
      ...
      const status = /approval required|denied/iu.test(wrapped.message)
        ? 403
        : 500;
      if (status === 500) {
        log.error(
  ...
  return new Promise((resolve, reject) => {
    const onError = (error: Error) => {
      ...
      reject(
        new ServerError(error.message, { cause: error, port: options.port })
      );
    };
    ...
    server.listen(options.port ?? 0, "127.0.0.1", () => {
      ...
      const handle: UiServerHandle = {
        ...
        getStreamSummary: () =>
          streamSummary
            ? { ...streamSummary, phases: [...streamSummary.phases] }
            : undefined,
        port: address.port,
