...
export const getUiStreamHostContext = (hostContext: Record<string, unknown> | undefined): UiStreamHostContext | undefined => {
    const candidate = hostContext?.[UI_STREAM_HOST_CONTEXT_KEY];
    const parsed = uiStreamHostContextSchema.safeParse(candidate);
    return parsed.success ? parsed.data : undefined;
};
export const getVisualizationStreamEnvelope = (structuredContent: unknown): VisualizationStreamEnvelope | undefined => {
