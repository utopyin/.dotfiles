...
const extractServers = (config: unknown, kind: ImportKind): Record<string, ServerEntry> => {
...
export const getServerProvenance = (overridePath?: string): Map<string, ServerProvenance> => {
            ...
            try {
                ...
                for (const name of Object.keys(servers)) {
                    if (!provenance.has(name)) {
                        provenance.set(name, { importKind, kind: "import", path: userPath });
                    }
                }
            }
            catch {
...
export const writeDirectToolsConfig = (changes: Map<string, true | string[] | false>, provenance: Map<string, ServerProvenance>, fullConfig: McpConfig): void => {
