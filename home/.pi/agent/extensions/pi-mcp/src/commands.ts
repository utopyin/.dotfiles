...
export const reconnectServers = async (
  state: McpExtensionState,
  ctx: ExtensionContext,
  targetServer?: string
): Promise<void> => {
  ...
  for (const [name, definition] of entries) {
    try {
      ...
      if (error instanceof NeedsAuthError) {
        ...
        if (ctx.hasUI) {
          ctx.ui.notify(
            `MCP: Failed to reconnect to ${name}: ${message}`,
            "error"
          );
        }
      }
...
export const authenticateServer = async (
  ...
  try {
    ...
    if (!result) {
      ...
      ctx.ui.notify(
        `"${serverName}" is already authenticated. Reconnecting...`,
        "info"
      );
      if (state) {
...
// ── MCP Panel ──────────────────────────────────────────────────────────────
export const openMcpPanel = async (
  state: McpExtensionState,
  pi: ExtensionAPI,
  ctx: ExtensionContext,
