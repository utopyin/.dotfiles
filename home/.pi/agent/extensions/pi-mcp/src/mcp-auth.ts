 ...
 */
const getForUrl = (serverName: string, serverUrl: string): McpAuthEntry | undefined => {
...
const set = (serverName: string, entry: McpAuthEntry, serverUrl?: string): void => {
    const data = readAll();
    if (serverUrl) {
...
const updateTokens = (serverName: string, tokens: McpAuthTokens, serverUrl?: string): void => {
    const entry = get(serverName) ?? {};
    entry.tokens = tokens;
    set(serverName, entry, serverUrl);
};
const updateClientInfo = (serverName: string, clientInfo: McpAuthClientInfo, serverUrl?: string): void => {
    const entry = get(serverName) ?? {};
 ...
 */
const getAuthStatus = (serverName: string): "authenticated" | "expired" | "not_authenticated" => {
    const entry = get(serverName);
    if (!entry?.tokens) {
