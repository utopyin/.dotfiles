...
class McpPanel {
	...
	private rebuildServerTools(
		...
		for (const tool of entry.tools ?? []) {
			...
			newTools.push({
				...
				wasDirect:
					prev !== undefined
						? (server.tools.find((t) => t.name === tool.name)?.wasDirect ??
							false)
						: false,
				estimatedTokens: estimateTokens(tool),
			});
		...
		for (const resource of entry.resources ?? []) {
			...
			newTools.push({
				...
				wasDirect:
					prev !== undefined
						? (server.tools.find((t) => t.name === baseName)?.wasDirect ??
							false)
						: false,
				estimatedTokens: estimateTokens(ct),
			});
	...
	render(width: number): string[] {
		...
		if (this.descSearchActive) {
			...
			lines.push(
				row(`${searchIcon}  ${fg(t.placeholder, italic("search..."))}`),
			);
		}
		...
		if (this.confirmingDiscard) {
			...
			const totalTokens = this.servers.reduce(
				(sum, s) =>
					...
					s.tools
						.filter((t) => t.isDirect)
						.reduce((ts, t) => ts + t.estimatedTokens, 0),
				0,
			);
			const stats =
				directCount > 0
