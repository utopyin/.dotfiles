...
export const openUrl = async (pi: ExtensionAPI, url: string, browser?: string): Promise<void> => {
    const os = platform();
    let result;
    if (os === "darwin") {
...
export const parallelLimit = async <T, R>(items: T[], limit: number, fn: (item: T) => Promise<R>): Promise<R[]> => {
    const results: R[] = [];
    let index = 0;
 ...
 */
export const extractToolUiStreamMode = (toolMeta: Record<string, unknown> | undefined): "eager" | "stream-first" | undefined => {
