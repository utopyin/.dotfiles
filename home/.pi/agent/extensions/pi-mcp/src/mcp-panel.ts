...
class McpPanel {
      ...
      this.visibleItems = this.visibleItems.filter((item) => {
        if (item.type === "server") {
          return this.visibleItems.some(
            (other) =>
              other.type === "tool" && other.serverIndex === item.serverIndex
          );
        }
        return true;
      });
  ...
  private updateDirty(): void {
    this.dirty = this.servers.some((s) =>
      s.tools.some((t) => t.isDirect !== t.wasDirect)
    );
  }
  private buildResult(): McpPanelResult {
    ...
    for (const server of this.servers) {
      ...
      if (
        directTools.length === server.tools.length &&
  ...
  private rebuildServerTools(
    ...
    for (const tool of entry.tools ?? []) {
      ...
      const wasDirect =
        server.tools.find((serverTool) => serverTool.name === tool.name)
          ?.wasDirect ?? false;
      newTools.push({
        description: tool.description ?? "",
        estimatedTokens: estimateTokens(tool),
    ...
    for (const resource of entry.resources ?? []) {
      ...
      const isDirect = prev ?? false;
      const wasDirect =
        server.tools.find((serverTool) => serverTool.name === baseName)
          ?.wasDirect ?? false;
      const ct: CachedTool = {
        description: resource.description,
        name: baseName,
  ...
  render(width: number): string[] {
    ...
    const row = (content: string) =>
      ...
      fg(t.border, "│");
    const emptyRow = () =>
      fg(t.border, "│") + " ".repeat(innerW) + fg(t.border, "│");
    ...
    if (this.descSearchActive) {
      lines.push(
        row(
          `${searchIcon}  ${fg(t.needsAuth, "desc:")} ${this.descQuery}${cursor}`
        )
      );
      ...
      lines.push(
        row(`${searchIcon}  ${fg(t.placeholder, ansiItalic("search..."))}`)
      );
    }
    lines.push(emptyRow());
    lines.push(divider());
    if (this.servers.length === 0) {
      ...
      for (let i = startIdx; i < endIdx; i += 1) {
        ...
        if (item.type === "server") {
          ...
          lines.push(
            row(
              this.renderToolRow(server.tools[item.toolIndex], isCursor, innerW)
            )
          );
        }
      }
      ...
      if (total > maxVis) {
        ...
        lines.push(
          row(
            `${rainbowProgress(prog, 10)}  ${fg(t.hint, `${this.cursorIndex + 1}/${total}`)}`
          )
        );
        lines.push(emptyRow());
      }
    ...
    if (this.confirmingDiscard) {
      ...
      const directCount = this.servers.reduce(
        (sum, server) =>
          sum + server.tools.filter((tool) => tool.isDirect).length,
        0
      );
      const totalTokens = this.servers.reduce(
        (sum, server) =>
          ...
          server.tools
            ...
            .reduce((tokenSum, tool) => tokenSum + tool.estimatedTokens, 0),
        0
      );
      const stats =
        directCount > 0
      ...
      lines.push(
        row(
          fg(
            t.description,
            stats + (this.dirty ? fg(t.needsAuth, "  (unsaved)") : "")
          )
        )
      );
    }
  ...
  private renderServerRow(server: ServerState, isCursor: boolean): string {
    ...
    const nameStr = isCursor
      ? ansiBold(fg(t.selected, server.name))
      : server.name;
    const importLabel =
  ...
  private renderToolRow(
    ...
    const descStr =
      maxDescLen > 5 && tool.description
        ? fg(
            t.description,
            `— ${truncateToWidth(tool.description, maxDescLen, "…")}`
          )
        : "";
