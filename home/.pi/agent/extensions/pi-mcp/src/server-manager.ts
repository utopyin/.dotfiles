...
import type { McpTool, McpResource, ServerDefinition, ServerStreamResultPatchNotification, Transport, } from "./types.js";
...
export class McpServerManager {
    ...
    private async createConnection(name: string, definition: ServerDefinition): Promise<ServerConnection> {
        ...
        try {
            if (definition.command) {
                ...
                transport = await McpServerManager.createHttpTransport(definition, name);
            }
            else {
        ...
        try {
            ...
            const [tools, resources] = await Promise.all([
                McpServerManager.fetchAllTools(client),
                McpServerManager.fetchAllResources(client),
            ]);
            return {
                client,
                definition,
    ...
    private static async createHttpTransport(definition: ServerDefinition, serverName?: string): Promise<Transport> {
        if (!definition.url) {
            throw new Error(`Server ${serverName ?? "?"} has no url`);
        }
        const urlText = definition.url;
        ...
        if (definition.auth === "bearer") {
            ...
            if (!token) {
                ...
                throw new Error(`Missing bearer token for MCP server "${serverName ?? urlText}" (${missingSource})`);
            }
        ...
        if (definition.auth === "oauth" && serverName) {
            authProvider = new McpOAuthProvider(serverName, urlText, {
        ...
        if (requestInit) {
            transportOpts.requestInit = requestInit;
        }
        const hasOpts = Object.keys(transportOpts).length > 0;
        // Try StreamableHTTP first, then SSE fallback
        const transports: {
            name: string;
            factory: () => StreamableHTTPClientTransport | SSEClientTransport;
        }[] = [
            {
        ...
        if (transportErrors.length > 0) {
            throw new Error(`All transports failed for "${serverName ?? urlText}": ${transportErrors.join(" | ")}`);
        }
        throw lastError ?? new Error("All transports failed");
    }
    private static async fetchAllTools(client: Client): Promise<McpTool[]> {
    ...
    private static async fetchAllResources(client: Client): Promise<McpResource[]> {
        try {
    ...
    async close(name: string): Promise<void> {
        ...
        await connection.client.close().catch(() => null);
...
const resolveEnv = (env?: Record<string, string>): Record<string, string> => {
    ...
    for (const [key, value] of Object.entries(env)) {
        ...
        resolved[key] = value
            .replaceAll(/\$\{(\w+)\}/gu, (_, name) => process.env[name] ?? "")
            .replaceAll(/\$env:(\w+)/gu, (_, name) => process.env[name] ?? "");
    }
    return resolved;
};
