...
export const showStatus = async (state: McpExtensionState, ctx: ExtensionContext): Promise<void> => {
    if (!ctx.hasUI) {
        return;
    }
...
export const showTools = async (state: McpExtensionState, ctx: ExtensionContext): Promise<void> => {
    if (!ctx.hasUI) {
        return;
    }
...
export const reconnectServers = async (state: McpExtensionState, ctx: ExtensionContext, targetServer?: string): Promise<void> => {
 ...
 */
async (serverName: string, definition: ServerEntry): Promise<{
 ...
 */
async (serverName: string, authorizationCode: string, transport: StreamableHTTPClientTransport): Promise<void> => {
 ...
 */
export const authenticateServer = async (serverName: string, config: McpConfig, ctx: ExtensionContext, state?: McpExtensionState): Promise<void> => {
    if (!ctx.hasUI) {
        return;
    }
    ...
    if (definition.auth !== "oauth") {
        ctx.ui.notify(`Server "${serverName}" does not use OAuth authentication.\n` +
            `Current auth mode: ${definition.auth ?? "none"}`, "error");
        return;
    }
    if (!definition.url) {
    ...
    try {
        ...
        try {
            ...
            ctx.ui.notify(`Could not open browser. Please open this URL manually:\n\n${authorizationUrl}`, "warning");
        }
        ...
        await pendingTransport?.close().catch(() => null);
    }
};
/**
 * Remove stored OAuth credentials for a server.
 */
export const removeAuth = async (serverName: string, ctx: ExtensionContext): Promise<void> => {
    if (!ctx.hasUI) {
        return;
    }
...
// ── MCP Panel ──────────────────────────────────────────────────────────────
export const openMcpPanel = async (state: McpExtensionState, pi: ExtensionAPI, ctx: ExtensionContext, configOverridePath?: string): Promise<void> => {
    const { config } = state;
    ...
    const callbacks: McpPanelCallbacks = {
        ...
        reconnect: async (serverName: string) => lazyConnect(state, serverName),
    ...
    const panelResult = await new Promise<McpPanelResult>((resolve) => {
        ctx.ui.custom((tui, _theme, _keybindings, done) => createMcpPanel(config, cache, provenanceMap, callbacks, tui, (result: McpPanelResult) => {
            done(result);
            resolve(result);
