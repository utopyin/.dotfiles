...
export const runToggleSkillsCommand = async (
	...
	try {
		...
		ctx.ui.notify(
			`Pi Skill Toggle failed to scan skills: ${
	...
	if (skills.length === 0) {
		ctx.ui.notify(
			"Pi Skill Toggle: no skills found in user or project skill directories",
			"info",
		);
		return;
	}
	...
	try {
		...
		ctx.ui.notify(
			`Pi Skill Toggle failed to plan changes: ${
				error instanceof Error ? error.message : String(error)
			}`,
			"error",
		);
...
const formatApplyResult = (result: ApplyResult): string => {
	const lines = [
		`Pi Skill Toggle applied ${result.applied.length} change${
