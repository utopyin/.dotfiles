...
export const initializeMcp = async (pi: ExtensionAPI, ctx: ExtensionContext): Promise<McpExtensionState> => {
    ...
    if (!cacheFileExists) {
        bootstrapAll = true;
        saveMetadataCache({ servers: {}, version: 1 });
    }
    else if (!cache) {
        cache = { servers: {}, version: 1 };
    ...
    for (const [name, definition] of serverEntries) {
        ...
        const lifecycleOptions = idleOverride === undefined ? undefined : { idleTimeout: idleOverride };
        lifecycle.registerServer(name, definition, lifecycleOptions);
    ...
    const results = await parallelLimit(startupServers, 10, async ([name, definition]) => {
        try {
            ...
            return { connection, definition, error: null, name, needsAuth: false };
        }
        catch (error) {
            if (error instanceof NeedsAuthError) {
                // Clean up the pending transport
                await error.transport.close().catch(() => null);
                return { connection: null, definition, error: null, name, needsAuth: true };
            }
            if (error instanceof NeedsClientRegistrationError) {
                return { connection: null, definition, error: error.message, name, needsAuth: false };
            }
            const message = error instanceof Error ? error.message : String(error);
            return { connection: null, definition, error: message, name, needsAuth: false };
        }
    });
...
export const updateServerMetadata = (state: McpExtensionState, serverName: string): void => {
...
export const updateMetadataCache = (state: McpExtensionState, serverName: string): void => {
    const connection = state.manager.getConnection(serverName);
    if (!connection || connection.status !== "connected") {
        return;
    }
    ...
    saveMetadataCache({ servers: { [serverName]: entry }, version: 1 });
};
export const flushMetadataCache = (state: McpExtensionState): void => {
...
export const updateStatusBar = (state: McpExtensionState): void => {
    const { ui } = state;
    if (!ui) {
        return;
    }
...
export const getFailureAgeSeconds = (state: McpExtensionState, serverName: string): number | null => {
...
export const lazyConnect = async (state: McpExtensionState, serverName: string): Promise<boolean> => {
    const connection = state.manager.getConnection(serverName);
    if (connection?.status === "connected") {
        updateServerMetadata(state, serverName);
        return true;
...
const getEffectiveIdleTimeoutMinutes = (state: McpExtensionState, serverName: string): number => {
    const definition = state.config.mcpServers[serverName];
    if (!definition) {
