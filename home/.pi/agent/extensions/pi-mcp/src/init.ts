...
export const initializeMcp = async (
  ...
  const idleSetting =
    typeof config.settings?.idleTimeout === "number"
  ...
  for (const [name, definition] of serverEntries) {
    ...
    const idleOverride =
      definition.idleTimeout ?? (lifecycleMode === "eager" ? 0 : undefined);
    ...
    if (
      cache?.servers?.[name] &&
      isServerCacheValid(cache.servers[name], definition)
    ) {
      const metadata = reconstructToolMetadata(
        name,
        cache.servers[name],
        prefix,
        definition.exposeResources
      );
  ...
  for (const {
    ...
    connection,
    error,
    needsAuth: serverNeedsAuth,
  } of results) {
    if (serverNeedsAuth) {
      needsAuth.add(name);
      if (ctx.hasUI) {
        ctx.ui.notify(
          `MCP: "${name}" requires authentication. Run /mcp-auth ${name}`,
          "warning"
        );
      }
      continue;
  ...
  if (envDirect !== "__none__") {
    ...
    if (missingCacheServers.length > 0) {
      const bootstrapResults = await parallelLimit(
        missingCacheServers.filter(
          (name) => !results.some((r) => r.name === name && r.connection)
        ),
        10,
        async (name) => {
      ...
      const bootstrapped = bootstrapResults
        .filter((r) => r.ok)
        .map((r) => r.name);
  ...
  lifecycle.setNeedsAuthCallback((serverName) => {
    ...
    if (ctx.hasUI) {
      ctx.ui.notify(
        `MCP: "${serverName}" requires authentication. Run /mcp-auth ${serverName}`,
        "warning"
      );
    }
  });
...
export const updateMetadataCache = (
  state: McpExtensionState,
  serverName: string
): void => {
  const connection = state.manager.getConnection(serverName);
...
export const getFailureAgeSeconds = (
  state: McpExtensionState,
  serverName: string
): number | null => {
...
export const lazyConnect = async (
  state: McpExtensionState,
  serverName: string
): Promise<boolean> => {
  const connection = state.manager.getConnection(serverName);
...
const getEffectiveIdleTimeoutMinutes = (
  state: McpExtensionState,
  serverName: string
): number => {
  const definition = state.config.mcpServers[serverName];
