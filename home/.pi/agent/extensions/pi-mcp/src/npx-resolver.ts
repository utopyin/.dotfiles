...
export const resolveNpxBinary = async (command: string, args: string[]): Promise<NpxResolution | null> => {
    let parsed: ParsedInvocation | null = null;
    if (command === "npx") {
        parsed = parseNpxArgs(args);
    }
    else if (command === "npm") {
        parsed = parseNpmExecArgs(args);
    }
    if (!parsed) {
        return null;
    }
...
const parseNpxArgs = (args: string[]): ParsedInvocation | null => {
    const separatorIndex = args.indexOf("--");
    const before = separatorIndex === -1 ? args : args.slice(0, separatorIndex);
    const after = separatorIndex === -1 ? [] : args.slice(separatorIndex + 1);
    ...
    if (sawPackageFlag) {
        const [binName] = positionals;
        if (!packageSpec || !binName) {
            return null;
        }
        const extraArgs = [...positionals.slice(1), ...after];
        return { binName, extraArgs, packageSpec };
    }
    const [packagePositional] = positionals;
    if (!packagePositional) {
        return null;
    }
    const extraArgs = [...positionals.slice(1), ...after];
    return { extraArgs, packageSpec: packagePositional };
};
const parseNpmExecArgs = (args: string[]): ParsedInvocation | null => {
    if (args[0] !== "exec") {
        return null;
    }
...
const resolveFromNpmCache = (packageSpec: string, binName?: string): NpxCacheEntry | null => {
    ...
    if (typeof binField === "string") {
        ...
        if (!binRel) {
            const [firstEntry] = Object.entries(binField);
            if (firstEntry) {
                [chosenBinName, binRel] = firstEntry;
            }
        }
    }
    if (!binRel) {
    ...
    if (!resolvedBin) {
        resolvedBin = resolvePath(packageDir, binRel);
...
const forceNpxCache = async (packageSpec: string): Promise<void> => {
    try {
...
const buildBinCandidates = (packageName: string, explicitBin?: string): string[] => {
    const candidates: string[] = [];
    if (explicitBin) {
        candidates.push(explicitBin);
    }
    if (packageName.startsWith("@")) {
        const [scope, namePart = ""] = packageName.split("/");
...
const extractPackageName = (spec: string): string | null => {
...
const defaultBinName = (packageName: string): string => {
    if (packageName.startsWith("@")) {
        const [, packagePart] = packageName.split("/");
        return packagePart ?? packageName.replace("@", "").replace("/", "-");
    }
    return packageName;
};
const findCachedPackageDir = (cacheDir: string, packageName: string): string | null => {
...
const findNodeModulesDir = (packageDir: string): string => {
...
const detectJsBinary = (binPath: string): boolean => {
...
const getNpmCacheDir = (): string | null => {
...
const saveCacheEntry = (key: string, entry: NpxCacheEntry): void => {
    const dir = dirname(CACHE_PATH);
    mkdirSync(dir, { recursive: true });
    const merged: NpxCache = { entries: {}, version: CACHE_VERSION };
    try {
        if (existsSync(CACHE_PATH)) {
...
const safeStatMtime = (path: string): number => {
    try {
