...
export class UiResourceHandler {
    ...
    async readUiResource(serverName: string, uri: string): Promise<UiResourceContent> {
        ...
        if (mimeType && !isHtmlMimeType(mimeType)) {
            ...
            throw new ResourceParseError(uri, `unsupported MIME type "${mimeType}" (expected text/html or ${RESOURCE_MIME_TYPE})`, { mimeType, server: serverName });
        }
...
const selectContent = (result: ReadResourceResult, preferredUri: string): ResourceContentRecord => {
...
const isHtmlMimeType = (mimeType: string): boolean => {
    const normalized = mimeType.toLowerCase();
    return normalized.startsWith("text/html") || normalized === RESOURCE_MIME_TYPE.toLowerCase();
};
const toHtml = (content: ResourceContentRecord): string => {
...
const extractUiMeta = (meta: Record<string, unknown> | undefined): UiResourceMeta => {
