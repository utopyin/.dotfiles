...
type UiPermissionPolicy = Record<string, never>;
export interface UiResourcePermissions {
    camera?: UiPermissionPolicy;
    microphone?: UiPermissionPolicy;
    geolocation?: UiPermissionPolicy;
    clipboardWrite?: UiPermissionPolicy;
}
 ...
 */
export const extractUiPromptText = (params: UiMessageParams): string | undefined => {
    ...
    if (params.role === "user" && Array.isArray(params.content)) {
        const text = params.content
            .map((block) => (block && typeof block === "object" && "text" in block ? String((block as {
            text?: unknown;
        }).text ?? "") : ""))
            .filter(Boolean)
            .join("\n\n");
 ...
 */
export const parseUiPromptHandoff = (prompt: string): UiPromptHandoff | undefined => {
    ...
    if (!/^[A-Za-z][A-Za-z0-9_-]*$/u.test(intent)) {
        return undefined;
    }
    try {
...
export interface ServerEntry {
    ...
    // Minutes, overrides global setting.
    idleTimeout?: number;
    // Resource handling
...
export interface McpSettings {
    ...
    // Minutes, default 10; 0 disables idle shutdown.
    idleTimeout?: number;
    directTools?: boolean;
}
// Root config
...
export interface ToolMetadata {
    ...
    resourceUri?: string;
    // For app-enabled tools: the UI resource URI.
 ...
 */
export const getServerPrefix = (serverName: string, mode: "server" | "none" | "short"): string => {
    if (mode === "none") {
        return "";
    }
    if (mode === "short") {
        let short = serverName.replace(/-?mcp$/iu, "").replaceAll("-", "_");
        if (!short) {
            short = "mcp";
        }
        return short;
 ...
 */
export const formatToolName = (toolName: string, serverName: string, prefix: "server" | "none" | "short"): string => {
