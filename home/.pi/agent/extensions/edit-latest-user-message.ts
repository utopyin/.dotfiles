...
type UserSessionMessageEntry = SessionMessageEntry & {
  message: Pick<UserMessage, "content" | "role">;
};

const getUserMessageText = (entry: UserSessionMessageEntry): string => {
...
): UserSessionMessageEntry | undefined => {
  ...
  pi.registerShortcut("alt+e", {
    ...
    handler: (ctx) => {
      ...
      if (existingText && existingText !== "/edit") {
        ctx.ui.notify(
          "Editor is not empty; not replacing it with /edit.",
          "warning"
        );
        return;
      }
