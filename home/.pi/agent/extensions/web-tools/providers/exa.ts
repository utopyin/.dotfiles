...
export class ExaSearchProvider implements SearchProvider {
  ...
  async search(
    ...
    const providerError = extractSearchErrorFromResponse(
      responseText,
      contentType
    );
    if (providerError) {
    ...
    if (results.length === 0 && !isExplicitNoResultsText(searchText)) {
      throw new Error(
        "Search provider returned an unrecognized response format"
      );
    }
...
): Exclude<SearchRequest["depth"], "deep"> =>
  depth === "deep" ? "fast" : depth;
