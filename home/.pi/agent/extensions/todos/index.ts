...
const formatTodoId = (id: string): string => `${TODO_ID_PREFIX}${id}`;
...
const displayTodoId = (id: string): string => formatTodoId(normalizeTodoId(id));
...
class TodoSelectorComponent extends Container implements Focusable {
  ...
  private updateList(): void {
    ...
    for (let i = startIndex; i < endIndex; i += 1) {
      ...
      const line = `${
        prefix + this.theme.fg("accent", formatTodoId(todo.id))
      } ${this.theme.fg(titleColor, todo.title || "(untitled)")}${this.theme.fg("muted", tagText)}${assignmentText} ${this.theme.fg(statusColor, `(${todo.status || "open"})`)}`;
  ...
  private moveSelection(delta: -1 | 1): void {
    ...
    if (delta < 0) {
      this.selectedIndex =
        this.selectedIndex === 0 ? lastIndex : this.selectedIndex - 1;
    } else {
      this.selectedIndex =
        this.selectedIndex === lastIndex ? 0 : this.selectedIndex + 1;
    }
    this.updateList();
  }
...
const garbageCollectTodos = async (
  todosDir: string,
...
const claimTodoAssignment = async (
  todosDir: string,
  id: string,
  ctx: ExtensionContext,
  force = false
...
const releaseTodoAssignment = async (
  todosDir: string,
  id: string,
  ctx: ExtensionContext,
  force = false
...
const handleTodoListAction = async (
  ...
  return todoToolListResult(
    "list",
    [...assignedTodos, ...openTodos],
    currentSessionId
  );
};

const handleTodoGetAction = async (
  todosDir: string,
  params: TodoToolParams
) => {
...
const prepareExistingTodoAction = (
  ...
  if (!existsSync(filePath)) {
    return {
      error: todoToolError(action, "not found", `Todo ${displayId} not found`),
    };
  }
...
const executeTodoToolAction = (
  ...
  switch (params.action) {
    ...
    default: {
      return todoToolError(
        params.action,
        `Unsupported todo action: ${params.action}`
      );
    }
  }
};
...
const todosExtension = (pi: ExtensionAPI) => {
  ...
  pi.registerTool({
    ...
    execute: (_toolCallId, params, _signal, _onUpdate, ctx) =>
      executeTodoToolAction(params as TodoToolParams, ctx),
    label: "Todo",
    name: "todo",
