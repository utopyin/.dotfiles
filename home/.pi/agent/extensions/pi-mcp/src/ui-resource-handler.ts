...
export class UiResourceHandler {
  ...
  async readUiResource(
    ...
    if (!uri.startsWith("ui://")) {
      throw new ResourceParseError(uri, "URI must start with ui://", {
        server: serverName,
      });
    }
    ...
    if (mimeType && !isHtmlMimeType(mimeType)) {
      ...
      throw new ResourceParseError(
        ...
        { mimeType, server: serverName }
      );
    }
    const html = toHtml(content);
    if (!html.trim()) {
      log.warn("Resource content is empty");
      throw new ResourceParseError(uri, "content is empty", {
        server: serverName,
      });
    }
...
const isHtmlMimeType = (mimeType: string): boolean => {
  ...
  return (
    ...
    normalized === RESOURCE_MIME_TYPE.toLowerCase()
  );
};
const toHtml = (content: ResourceContentRecord): string => {
  ...
  throw new Error(
    `UI resource ${content.uri ?? "(unknown)"} did not include text or blob content`
  );
};
const extractUiMeta = (
