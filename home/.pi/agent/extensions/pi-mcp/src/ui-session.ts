...
export const maybeStartUiSession = async (
  state: McpExtensionState,
  ...
  try {
    if (
      ...
      if (streamToken) {
        state.manager.registerUiStreamListener(
          streamToken,
          (serverName, notification) => {
            ...
            existingHandle.sendResultPatch(
              withStreamEnvelope(
                notification.result as CallToolResult,
                streamId,
                nextStreamSequence
              )
            );
      ...
      return {
        ...
        sendResultPatch: (result: CallToolResult) => {
          ...
          nextStreamSequence += 1;
          existingHandle.sendResultPatch(
            withStreamEnvelope(result, streamId, nextStreamSequence)
          );
        },
      ...
      onMessage: (params: UiMessageParams) => {
        ...
        if (prompt) {
          if (state.sendMessage) {
            ...
            log.debug("Triggered agent turn for UI prompt", {
          ...
          if (intent && state.sendMessage) {
            const paramsStr = intentParams
              ? ` ${JSON.stringify(intentParams)}`
              : "";
            state.sendMessage(
              {
                content: [
    ...
    const useGlimpse =
      viewerPref === "glimpse" || (viewerPref !== "browser" && glimpseDetected);
    if (useGlimpse) {
      try {
